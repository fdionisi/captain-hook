...
download() {
  ...
  printf "\n" >&2
  info "This is likely due to Captain Hook not yet supporting your configuration."
...
install() {
  ...
  if test_writeable "${BIN_DIR}"; then
    sudo=""
    msg="Installing Captain Hook, please wait…"
  else
    warn "Escalated permissions are required to install to ${BIN_DIR}"
    elevate_priv
    sudo="sudo"
    msg="Installing Captain Hook as root, please wait…"
  fi
  info "$msg"
...
is_build_available() {
  ...
  if [ "${good}" != "1" ]; then
    error "${arch} builds for ${platform} are not yet available for Captain Hook"
    printf "\n" >&2
    info "If you would like to see a build for your configuration,"
    info "please create an issue requesting a build for ${MAGENTA}${target}${NO_COLOR}:"
    info "${BOLD}${UNDERLINE}https://github.com/fdionisi/captain-hook/issues/new/${NO_COLOR}"
...
confirm "Install Captain Hook ${GREEN}latest${NO_COLOR} to ${BOLD}${GREEN}${BIN_DIR}${NO_COLOR}?"
check_bin_dir "${BIN_DIR}"

install "${EXT}"
completed "Captain Hook installed"